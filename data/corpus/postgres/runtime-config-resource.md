...
| --- | --- | --- | --- | --- |
| [Prev](runtime-config-connection.html "19.3. Connections and Authentication") | [Up](runtime-config.html "Chapter 19. Server Configuration") | Chapter 19. Server Configuration | [Home](index.html "PostgreSQL 17.10 Documentation") | [Next](runtime-config-wal.html "19.5. Write Ahead Log") |

---

## 19.4. Resource Consumption [#](#RUNTIME-CONFIG-RESOURCE)

[19.4.1. Memory](runtime-config-resource.html#RUNTIME-CONFIG-RESOURCE-MEMORY)

[19.4.2. Disk](runtime-config-resource.html#RUNTIME-CONFIG-RESOURCE-DISK)

[19.4.3. Kernel Resource Usage](runtime-config-resource.html#RUNTIME-CONFIG-RESOURCE-KERNEL)

[19.4.4. Cost-based Vacuum Delay](runtime-config-resource.html#RUNTIME-CONFIG-RESOURCE-VACUUM-COST)

[19.4.5. Background Writer](runtime-config-resource.html#RUNTIME-CONFIG-RESOURCE-BACKGROUND-WRITER)

[19.4.6. Asynchronous Behavior](runtime-config-resource.html#RUNTIME-CONFIG-RESOURCE-ASYNC-BEHAVIOR)
...
:   Controls whether huge pages are requested for the main shared memory area. Valid values are `try` (the default), `on`, and `off`. This parameter can only be set at server start. With `huge_pages` set to `try`, the server will try to request huge pages, but fall back to the default if that fails. With `on`, failure to request huge pages will prevent the server from starting up. With `off`, huge pages will not be requested. The actual state of huge pages is indicated by the server variable [huge\_pages\_status](runtime-config-preset.html#GUC-HUGE-PAGES-STATUS).
    ...
    The use of huge pages results in smaller page tables and less CPU time spent on memory management, increasing performance. For more details about using huge pages on Linux, see [Section 18.4.5](kernel-resources.html#LINUX-HUGE-PAGES "18.4.5. Linux Huge Pages").
...
:   Controls the size of huge pages, when they are enabled with [huge\_pages](runtime-config-resource.html#GUC-HUGE-PAGES). The default is zero (`0`). When set to `0`, the default huge page size on the system will be used. This parameter can only be set at server start.
    ...
    Some commonly available page sizes on modern 64 bit server architectures include: `2MB` and `1GB` (Intel and AMD), `16MB` and `16GB` (IBM POWER), and `64kB`, `2MB`, `32MB` and `1GB` (ARM). For more information about usage and support, see [Section 18.4.5](kernel-resources.html#LINUX-HUGE-PAGES "18.4.5. Linux Huge Pages").
...
:   Specifies the amount of memory to use to cache the contents of `pg_commit_ts` (see [Table 65.1](storage-file-layout.html#PGDATA-CONTENTS-TABLE "Table 65.1. Contents of PGDATA")). If this value is specified without units, it is taken as blocks, that is `BLCKSZ` bytes, typically 8kB. The default value is `0`, which requests `shared_buffers`/512 up to 1024 blocks, but not fewer than 16 blocks. This parameter can only be set at server start.

`multixact_member_buffers` (`integer`) [#](#GUC-MULTIXACT-MEMBER-BUFFERS)
:   Specifies the amount of shared memory to use to cache the contents of `pg_multixact/members` (see [Table 65.1](storage-file-layout.html#PGDATA-CONTENTS-TABLE "Table 65.1. Contents of PGDATA")). If this value is specified without units, it is taken as blocks, that is `BLCKSZ` bytes, typically 8kB. The default value is `32`. This parameter can only be set at server start.

`multixact_offset_buffers` (`integer`) [#](#GUC-MULTIXACT-OFFSET-BUFFERS)
:   Specifies the amount of shared memory to use to cache the contents of `pg_multixact/offsets` (see [Table 65.1](storage-file-layout.html#PGDATA-CONTENTS-TABLE "Table 65.1. Contents of PGDATA")). If this value is specified without units, it is taken as blocks, that is `BLCKSZ` bytes, typically 8kB. The default value is `16`. This parameter can only be set at server start.

`notify_buffers` (`integer`) [#](#GUC-NOTIFY-BUFFERS)
:   Specifies the amount of shared memory to use to cache the contents of `pg_notify` (see [Table 65.1](storage-file-layout.html#PGDATA-CONTENTS-TABLE "Table 65.1. Contents of PGDATA")). If this value is specified without units, it is taken as blocks, that is `BLCKSZ` bytes, typically 8kB. The default value is `16`. This parameter can only be set at server start.

`serializable_buffers` (`integer`) [#](#GUC-SERIALIZABLE-BUFFERS)
:   Specifies the amount of shared memory to use to cache the contents of `pg_serial` (see [Table 65.1](storage-file-layout.html#PGDATA-CONTENTS-TABLE "Table 65.1. Contents of PGDATA")). If this value is specified without units, it is taken as blocks, that is `BLCKSZ` bytes, typically 8kB. The default value is `32`. This parameter can only be set at server start.

`subtransaction_buffers` (`integer`) [#](#GUC-SUBTRANSACTION-BUFFERS)
:   Specifies the amount of shared memory to use to cache the contents of `pg_subtrans` (see [Table 65.1](storage-file-layout.html#PGDATA-CONTENTS-TABLE "Table 65.1. Contents of PGDATA")). If this value is specified without units, it is taken as blocks, that is `BLCKSZ` bytes, typically 8kB. The default value is `0`, which requests `shared_buffers`/512 up to 1024 blocks, but not fewer than 16 blocks. This parameter can only be set at server start.

`transaction_buffers` (`integer`) [#](#GUC-TRANSACTION-BUFFERS)
:   Specifies the amount of shared memory to use to cache the contents of `pg_xact` (see [Table 65.1](storage-file-layout.html#PGDATA-CONTENTS-TABLE "Table 65.1. Contents of PGDATA")). If this value is specified without units, it is taken as blocks, that is `BLCKSZ` bytes, typically 8kB. The default value is `0`, which requests `shared_buffers`/512 up to 1024 blocks, but not fewer than 16 blocks. This parameter can only be set at server start.
...
`file_extend_method` (`enum`) [#](#GUC-FILE-EXTEND-METHOD)
...
`max_notify_queue_pages` (`integer`) [#](#GUC-MAX-NOTIFY-QUEUE-PAGES)
:   Specifies the maximum amount of allocated pages for [NOTIFY](sql-notify.html "NOTIFY") / [LISTEN](sql-listen.html "LISTEN") queue. The default value is 1048576. For 8 KB pages it allows to consume up to 8 GB of disk space. This parameter can only be set at server start.
...
### 19.4.4. Cost-based Vacuum Delay [#](#RUNTIME-CONFIG-RESOURCE-VACUUM-COST)
...
### 19.4.5. Background Writer [#](#RUNTIME-CONFIG-RESOURCE-BACKGROUND-WRITER)
...
### 19.4.6. Asynchronous Behavior [#](#RUNTIME-CONFIG-RESOURCE-ASYNC-BEHAVIOR)
...
`io_combine_limit` (`integer`) [#](#GUC-IO-COMBINE-LIMIT)
...
|  |  |  |
| --- | --- | --- |
| [Prev](runtime-config-connection.html "19.3. Connections and Authentication") | [Up](runtime-config.html "Chapter 19. Server Configuration") | [Next](runtime-config-wal.html "19.5. Write Ahead Log") |
| 19.3. Connections and Authentication | [Home](index.html "PostgreSQL 17.10 Documentation") | 19.5. Write Ahead Log |
