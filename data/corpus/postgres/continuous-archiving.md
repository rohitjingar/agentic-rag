...
| --- | --- | --- | --- | --- |
| [Prev](backup-file.html "25.2. File System Level Backup") | [Up](backup.html "Chapter 25. Backup and Restore") | Chapter 25. Backup and Restore | [Home](index.html "PostgreSQL 17.10 Documentation") | [Next](high-availability.html "Chapter 26. High Availability, Load Balancing, and Replication") |

---

## 25.3. Continuous Archiving and Point-in-Time Recovery (PITR) [#](#CONTINUOUS-ARCHIVING)

[25.3.1. Setting Up WAL Archiving](continuous-archiving.html#BACKUP-ARCHIVING-WAL)

[25.3.2. Making a Base Backup](continuous-archiving.html#BACKUP-BASE-BACKUP)

[25.3.3. Making an Incremental Backup](continuous-archiving.html#BACKUP-INCREMENTAL-BACKUP)

[25.3.4. Making a Base Backup Using the Low Level API](continuous-archiving.html#BACKUP-LOWLEVEL-BASE-BACKUP)

[25.3.5. Recovering Using a Continuous Archive Backup](continuous-archiving.html#BACKUP-PITR-RECOVERY)

[25.3.6. Timelines](continuous-archiving.html#BACKUP-TIMELINES)

[25.3.7. Tips and Examples](continuous-archiving.html#BACKUP-TIPS)

[25.3.8. Caveats](continuous-archiving.html#CONTINUOUS-ARCHIVING-CAVEATS)
...
Instead of taking a full or incremental base backup using [pg\_basebackup](app-pgbasebackup.html "pg_basebackup"), you can take a base backup using the low-level API. This procedure contains a few more steps than the pg\_basebackup method, but is relatively simple. It is very important that these steps are executed in sequence, and that the success of a step is verified before proceeding to the next step.
...
3. Perform the backup, using any convenient file-system-backup tool such as tar or cpio (not pg\_dump or pg\_dumpall). It is neither necessary nor desirable to stop normal operation of the database while you do this. See [Section 25.3.4.1](continuous-archiving.html#BACKUP-LOWLEVEL-BASE-BACKUP-DATA "25.3.4.1. Backing Up the Data Directory") for things to consider during this backup.
...
8. Set recovery configuration settings in `postgresql.conf` (see [Section 19.5.5](runtime-config-wal.html#RUNTIME-CONFIG-WAL-ARCHIVE-RECOVERY "19.5.5. Archive Recovery")) and create a file `recovery.signal` in the cluster data directory. You might also want to temporarily modify `pg_hba.conf` to prevent ordinary users from connecting until you are sure the recovery was successful.
